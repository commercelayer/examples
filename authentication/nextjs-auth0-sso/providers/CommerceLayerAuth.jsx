...
export const CommerceLayerAuthProvider = ({ children }) => {
      ...
      if (!isLoading){
        if (user) {
          ...
          if (isValid(auth)){
            ...
            const response = await fetch('api/token').then((response) => response.json())

            setAuth(storeAuth('customer', {...response, expires: new Date(response.expires)}))
          }
        } else {
          ...
          if (isValid(auth)){
            ...
            const salesChannelToken = await getSalesChannelToken({
            ...
            console.log("You need to add those values on your .env.local file:")
            console.log(`NEXT_PUBLIC_CL_ORGANIZATION_ID=${payload.organization.id}`)
            console.log(`NEXT_PUBLIC_CL_SALES_CHANNEL_ID=${payload.application.id}`)
            console.log(`NEXT_PUBLIC_CL_PRICE_LIST_ID=${payload.market.price_list_id}`)
            console.log(`NEXT_PUBLIC_CL_STOCK_LOCATION_IDS=${JSON.stringify(payload.market.stock_location_ids)}`)
            if (token) {
  ...
  return (
    <CommerceLayerAuthContext.Provider value={{ auth, isLoading: isLoadingAuth }}>
      {children}
