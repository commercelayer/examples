...
app.post("/callback", (req, res) => {
  ...
  if (req.method === "POST" && signature === encode) {
    ...
    const inputData = {
      ...
      shipmentNumber:
        payload.included.find((x) => x.type === "shipments")?.attributes
          ?.number ?? "no shipment required",
      shippingMethod:
        payload.included.find((x) => x.type === "shipping_methods")?.attributes
          ?.name ?? "no shipment required",
      lineItemsHtml:
